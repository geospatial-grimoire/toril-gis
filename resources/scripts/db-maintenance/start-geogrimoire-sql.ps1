# This Powershell script starts the Cloud SQL instance for Geospatial Grimoire
...
# If the instance is already running, exit
if ($status -eq "RUNNABLE") {
    Write-Host "✅ Cloud SQL instance '$INSTANCE_NAME' is already RUNNING!"
    Notify-Success "Cloud SQL instance is already running."
    exit 0
}

# If the instance is STOPPED, start it
if ($status -eq "STOPPED") {
    Write-Host "🚀 Starting Cloud SQL instance '$INSTANCE_NAME'..."
    $startTime = Get-Date  # Start time measurement
    & $GCLOUD_PATH sql instances patch $INSTANCE_NAME --activation-policy=ALWAYS --project=$PROJECT_ID
} else {
    Write-Host "⚠️ Cloud SQL instance is in an unknown state: $status"
    Notify-Error "Cloud SQL startup failed. Unknown state: $status"
    exit 1
}

# Wait until the instance is RUNNABLE (NO TIME CONSTRAINT)
Write-Host "⏳ Waiting indefinitely for Cloud SQL to become RUNNABLE..."
...
Write-Host "✅ Cloud SQL instance '$INSTANCE_NAME' is now RUNNING!"
Write-Host "⏱️ Time taken: $timeTakenMinutes minutes and $remainingSeconds seconds"

Notify-Success "Cloud SQL is now running! Time taken: $timeTakenMinutes min $remainingSeconds sec."
exit 0
