...
GeoJSON is a text-based vector format. Each layer is exported as a separate `.geojson` file. Suitable for web developers, GIS tools (QGIS, ArcGIS), and data processing scripts.

## Files included

- `*.geojson` — vector layers (points, lines, polygons)
- `README.md` — this documentation

## How to use

Open any `.geojson` file in QGIS (drag & drop), Mapshaper (mapshaper.org), or any text editor. GeoJSON files contain data only; QGIS project styling and visualization settings are not included, so set up layer styling yourself when using this format.
