...
SVG export writes one vector overlay file per layer. Every file uses the same canvas size and viewBox, so layers can be stacked in Illustrator or other vector editors without manual alignment.

## Files included

- `layers/*.svg` — one SVG overlay per exported layer, with embedded attribution metadata
- `README.md` — this documentation

## How to use

Import multiple files from `layers/` into the same Illustrator document. They share width, height, and viewBox, so align them at the same origin to overlay the map layers. SVG files are pre-rendered vector overlays, not GIS datasets; use GeoPackage, GeoJSON, or Shapefile exports when you need editable GIS attributes and layer filtering.
