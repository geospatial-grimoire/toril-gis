...
ESRI Shapefile is a legacy vector GIS format. DBF field names are limited to 10 characters and DBF text values are shortened for UTF-8 safety. This export uses an explicit field_map.json instead of relying on GDAL field-name laundering. Use GeoPackage or GeoJSON when preserving full attributes matters.

## Files included

- `layers/*.shp` plus sidecar files — one Shapefile dataset per exported layer
- `field_map.json` — mapping from source fields to DBF-safe field names
- `README.md` — this documentation

## How to use

Open the `layers/` folder in QGIS, ArcGIS, or other desktop GIS software. Keep each `.shp` file together with its `.dbf`, `.shx`, `.prj`, and `.cpg` sidecar files. Shapefiles contain data only; QGIS project styling and visualization settings are not included, so set up layer styling yourself when using this format.
